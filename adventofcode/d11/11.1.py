...
def count_adjacent_seats(seats: List[str], i: int, j: int) -> Tuple[int, int, int]:
    """
    This function counts all types of adjacent seats
    """
    ...
    adj_idx = list(product([-1, 0, 1], repeat=2))  # O(1)
    adj_idx.remove((0, 0))
    for idx in adj_idx:  # O(1)
        if len(seats) > i + idx[0] >= 0 and len(seats[j]) > j + idx[1] >= 0:
            curr_neighbour = seats[i + idx[0]][j + idx[1]]
...
def apply_single_iteration(lines: List[str]) -> Tuple[List[str], bool]:
    ...
    for i, line in enumerate(lines):  # O(m)
        for j, seat in enumerate(line):  # O(k)
            occupied, free, floor = count_adjacent_seats(lines, i, j)
            if occupied == 0 and lines[i][j] == FREE:
                updated_lines[i] = updated_lines[i][:j] + OCCUPIED + updated_lines[i][j + 1:]
                any_modifications = True
            elif occupied >= 4 and lines[i][j] == OCCUPIED:
                updated_lines[i] = updated_lines[i][:j] + 'L' + updated_lines[i][j + 1:]
                any_modifications = True
            else:
...
def main():
    ...
    flag = True
    while flag:
        lines, modifications = apply_single_iteration(lines)
        flag = modifications
